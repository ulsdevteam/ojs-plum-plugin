...
{if $plumHtmlPrefix}{$plumHtmlPrefix}{/if}
 <!-- Plum Analytics -->
<a href="https://plu.mx/a/?doi={$plumSubmissionDOI|escape}" class="{$plumWidgetType|escape}" {if $plumHideWhenEmpty}data-hide-when-empty="true" {/if}{if $plumHidePrint}data-hide-print="true" {/if}{if $plumOrientation}data-orientation="{$plumOrientation|escape}" {/if}{if $plumPopup}data-popup="{$plumPopup|escape}" {/if}{if $plumBorder}data-border="{$plumBorder|escape}"{/if}{if $plumWidth}data-width="{$plumWidth|escape}"{/if}></a>
<!-- /Plum Analytics -->
